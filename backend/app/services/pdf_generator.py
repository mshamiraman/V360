...
class PDFReportGenerator:
    ...
    def _build_title_page(self, scan_data: Dict[str, Any]) -> List:
        ...
        report_data = [
            ...
            ['AI Model:', 'Gemini-2.5-flash:Enhanced']
        ]
    ...
    def _build_executive_summary(self, vulnerabilities: List[Dict[str, Any]]) -> List:
        ...
        risk_data = [
            ...
            ['Critical', str(critical), f"{(critical/total_vulns*100):.1f}%" if total_vulns >
             0 else "0%", 'Immediate Action Required'],
            ['High', str(high), f"{(high/total_vulns*100):.1f}%" if total_vulns >
             0 else "0%", 'Address Within 48 Hours'],
            ['Medium', str(medium), f"{(medium/total_vulns*100):.1f}%" if total_vulns >
             0 else "0%", 'Address Within 1 Week'],
            ['Low', str(low), f"{(low/total_vulns*100):.1f}%" if total_vulns >
             0 else "0%", 'Address During Maintenance']
        ]
    ...
    def _build_vulnerability_overview(self, vulnerabilities: List[Dict[str, Any]]) -> List:
        ...
        story.append(Paragraph("Vulnerability Overview",
                     self.styles['SectionHeader']))
        ...
        service_data = [['Service', 'Version', 'Port',
    ...
    def _build_detailed_vulnerabilities(self, vulnerabilities: List[Dict[str, Any]]) -> List:
        ...
        story.append(Paragraph("Detailed Vulnerability Analysis",
                     self.styles['SectionHeader']))
        ...
        for i, vuln in enumerate(sorted_vulns, 1):
            ...
            formatted_description = self._format_ai_content_for_pdf(
                description)
            story.append(
                Paragraph(f"<b>Description:</b>", self.styles['Normal']))
            ...
            formatted_recommendation = self._format_ai_content_for_pdf(
                recommendation)
            story.append(
                Paragraph(f"<b>AI-Enhanced Recommendations:</b>", self.styles['Normal']))
            story.append(Spacer(1, 4))
            story.append(
            ...
            if commands:
                story.append(
                    Paragraph("<b>Remediation Commands:</b>", self.styles['Normal']))
                story.append(Spacer(1, 8))
                ...
                for cmd in commands[:3]:  # Limit to 3 commands per vulnerability
                    ...
                    if requires_sudo:
                        header_parts.append(
                            "<font color='#d32f2f'><b>[REQUIRES SUDO]</b></font>")
                    if is_destructive:
                        header_parts.append(
                            "<font color='#d32f2f'><b>[DESTRUCTIVE]</b></font>")
    ...
    def _build_ai_insights_section(self, vulnerabilities: List[Dict[str, Any]]) -> List:
        ...
        story.append(Paragraph("AI-Enhanced Security Insights",
                     self.styles['SectionHeader']))
        ...
        if critical_vulns or high_vulns:
            story.append(Paragraph("🤖 AI Risk Prioritization",
        ...
        story.append(
            Paragraph("🧠 Intelligent Remediation Strategy", self.styles['Heading3']))
        story.append(Spacer(1, 8))
        ...
        # Technical Insights
        story.append(Paragraph("⚙️ Technical Analysis Highlights",
                     self.styles['Heading3']))
        story.append(Spacer(1, 8))
    ...
    def _build_patch_recommendations(self, vulnerabilities: List[Dict[str, Any]]) -> List:
        ...
        story.append(Paragraph("Patch Management Recommendations",
                     self.styles['SectionHeader']))
        ...
        if critical_vulns:
            story.append(
                Paragraph("🔴 IMMEDIATE ACTION REQUIRED (24 hours)", self.styles['HighRisk']))
            story.append(Spacer(1, 8))
        ...
        if high_vulns:
            story.append(Paragraph("🟡 HIGH PRIORITY (48-72 hours)",
